...
class Peeker extends Connect
{
    ...
    public $message_class = 'Deltagestor\Peeker\parts';
    ...
    public function message($start, $end = null)
    {
        ...
        if ($start === 0) {
            $this->log_state('ERROR: MsgNo cannot be 0. Mail server MsgNo starts at 1.');
        }
        ...
        while ($this->current_id++ < $this->end_id) {
            ...
            if ($this->detectors_on) {
                ...
                $this->detector_set->set_log_array([]);
     ...
     */
    public function set_message_class($class_name): void
    {
     ...
     */
    public function flag_mail($id_or_range, $flag_string, $set_flag = true): void
    {
        if ($set_flag) {
     ...
     */
    public function move_mail($id_or_range, $mailbox_name): void
    {
     ...
     */
    public function delete($imap_range_string): void
    {
    ...
    public function close($expunge_override = false)
    {
        if (is_resource($this->resource)) {
            ...
            if ($this->expunge_on_close or $expunge_override) {
                $this->expunge();
            }
    ...
    public function get_attachment_dir()
    {
        ...
        $class_var_defaults = get_class_vars(static::class);
     ...
     */
    public function layer($layer): void
    {
        // tell the object we are using layers
        //$this->layers = TRUE;
        $layer_name = $layer . 'Methods';
    ...
    public function make_detector_set()
    {
        ...
        if (! isset($this->detector_set)) {
            $this->detector_set = new DetectorSet();
        }
     ...
     */
    private function _set_start_and_end_ids($start = null, $end = null): void
    {
            ...
            if (preg_match('/^([^:\s]+):\s(.+)/', $line, $m)) {
                ...
                if (! isset($new_header_array[$current_header])) {
                    ...
                    if (! is_array($new_header_array[$current_header])) {
                        ...
                        $new_header_array[$current_header] = [$new_header_array[$current_header], $current_data];
                    } else { // if it is already an array then append an array item
                        // this runs when a header name is repeated
                        // (like Received often is)
